...
namespace BlackCat\Database\Packages\GlobalIdRegistry\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\GlobalIdRegistry\Dto\GlobalIdRegistryDto;
use BlackCat\Database\Packages\GlobalIdRegistry\Mapper\GlobalIdRegistryDtoMapper;
use BlackCat\Database\Packages\GlobalIdRegistry\Repository\GlobalIdRegistryRepository;
 ...
 */
final class GlobalIdRegistryAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private GlobalIdRegistryRepository $globalIdRegistryRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'global_id_registry' => $this->globalIdRegistryRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\GlobalIdRegistry\ModuleException("Record {$idStr} not found for locking.");
            }
