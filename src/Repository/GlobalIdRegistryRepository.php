...
namespace BlackCat\Database\Packages\GlobalIdRegistry\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\GlobalIdRegistry\Definitions;
use BlackCat\Database\Packages\GlobalIdRegistry\Criteria;
use BlackCat\Database\Packages\GlobalIdRegistry\Dto\GlobalIdRegistryDto as Dto;
use BlackCat\Database\Packages\GlobalIdRegistry\Mapper\GlobalIdRegistryDtoMapper as RowMapper;
...
use BlackCat\Database\Packages\GlobalIdRegistry\Repository\GlobalIdRegistryRepositoryInterface;
...
class GlobalIdRegistryRepository implements GlobalIdRegistryRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'gid' ];
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\GlobalIdRegistry\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'guid', 'entity_table', 'entity_pk' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\GlobalIdRegistry\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'guid', 'entity_table', 'entity_pk' ]);
          return count($rows);
        }
    ...
    public function getByEntityTableAndEntityPk(string $entityTable, string $entityPk, bool $asDto = false): array|\BlackCat\Database\Packages\GlobalIdRegistry\Dto\GlobalIdRegistryDto|null {
        return $this->getByUnique([ 'entity_table' => $entityTable, 'entity_pk' => $entityPk ], $asDto);
    }
    public function existsByEntityTableAndEntityPk(string $entityTable, string $entityPk): bool {
        $where = 't.' . Ident::q($this->db, 'entity_table') . ' = :uniq_entity_table' . ' AND ' . 't.' . Ident::q($this->db, 'entity_pk') . ' = :uniq_entity_pk';
        return $this->exists($where, [ 'uniq_entity_table' => $entityTable, 'uniq_entity_pk' => $entityPk ]);
    }
    /** @return int|string|null */
    public function getIdByEntityTableAndEntityPk(string $entityTable, string $entityPk) {
        $row = $this->getByEntityTableAndEntityPk($entityTable, $entityPk, false);
