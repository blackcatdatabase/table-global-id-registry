...
final class GlobalIdRegistryRepository implements RepoContract, KeysetRepoContract
{
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'gid' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\GlobalIdRegistry\Definitions::table());
            return $bulk->upsertMany($rows, [ 'gid' ], [ 'guid', 'entity_table', 'entity_pk' ]);
        }
    ...
    */
    ...
    public function getByEntityTableAndEntityPk(string $entityTable, string $entityPk, bool $asDto = false): array|\BlackCat\Database\Packages\GlobalIdRegistry\Dto\GlobalIdRegistryDto|null {
        $row = $this->getByUnique([ 'entity_table' => $entityTable, 'entity_pk' => $entityPk ]);
        if (!$asDto || !$row) return $row;
