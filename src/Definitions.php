...
final class Definitions {
    // --- core metadata ---
    public static function table(): string { return 'global_id_registry'; }
    public static function contractView(): string { return 'vw_global_id_registry'; }
    /** @return string[] */
    public static function columns(): array { return [ 'gid', 'guid', 'entity_table', 'entity_pk', 'created_at' ]; }
     ...
     * gid may be "id" or "col1, col2".
     * @return string[]
     */
    public static function pkColumns(): array {
        $raw = trim('gid');
        if ($raw === '') {
            throw new \InvalidArgumentException('Definitions::pkColumns(): token gid must not be empty.');
        }
    ...
    public static function defaultOrder(): ?string {
        $c = trim('created_at DESC, gid DESC'); return $c !== '' ? $c : null;
    }

    /** @return array<int,array<int,string>> list of unique keys */
    public static function uniqueKeys(): array { return [ [ 'entity_table', 'entity_pk' ], [ 'gid' ] ]; }
